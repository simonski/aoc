...
func AOC_2020_13_part1_attempt1(cli *goutils.CLI) {
	...
	startTime, buses := ParseDay13File(filename)
	fmt.Printf("time=%v\nbuses=%v\n", startTime, buses)

	earliest, bus := Day13Logic(startTime, buses)
	waitTime := earliest - startTime
	fmt.Printf("For time %v, earliest bus is %v, time to wait is %v, %v * %v = %v\n", startTime, bus, waitTime, bus, waitTime, bus*waitTime)
...
func ParseDay13File(filename string) (int, []int) {
