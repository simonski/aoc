...
func (c *Chamber) RunPart1(VERBOSE bool, VERY_VERBOSE bool, breakAfterRock int, findFloor bool) {
	rock := c.NewRock()
	c.AddRock(rock)
	if VERBOSE {
		fmt.Printf("\n[%v] %v\n%v\n", 0, "BEGIN", c.Debug())
	...
	for {
		...
		if !c.TickPart1(instruction, VERBOSE, VERY_VERBOSE) {
			c.CurrentRock = nil
			...
			if findFloor && c.RockCount%1000 == 0 {
				max := 1000000000000
...
func (c *Chamber) TickPart1(instruction string, VERBOSE bool, VERY_VERBOSE bool) bool {
	if instruction == "<" {
